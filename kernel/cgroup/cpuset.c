 ...
 * guarantee_online_mems guarantees at least one N_MEMORY node is set.
 */
static int cpuset_spread_node(int *rotor)
{
	int node;

	/* skip private nodes; MAX_NUMNODES>1 guard avoids a !NUMA spin */
	do {
		node = next_node_in(*rotor, current->mems_allowed);
		*rotor = node;
	} while (MAX_NUMNODES > 1 && node_state(node, N_MEMORY_PRIVATE));
